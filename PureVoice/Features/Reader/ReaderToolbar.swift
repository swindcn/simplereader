...
struct ReaderToolbar: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack {
                iconButton("chevron.backward", label: "返回书架", identifier: "reader.back", action: onBack)
                Spacer()
                iconButton("list.bullet", label: "目录", identifier: "reader.tableOfContents", action: onTableOfContents)
            }
