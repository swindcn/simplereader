...
struct EPUBBuilder: Sendable {
    ...
    func build(
        ...
        try await addXML(packageDocument(chapters: normalizedChapters, title: normalizedTitle, author: normalizedAuthor, language: normalizedLanguage), path: "EPUB/package.opf", to: archive)
    ...
    private func packageDocument(chapters: [Chapter], title: String, author: String, language: String) -> String {
    ...
    private func add(_ data: Data, path: String, compression: CompressionMethod, to archive: Archive) async throws {
        try await archive.addEntry(with: path, type: .file, uncompressedSize: Int64(data.count), compressionMethod: compression) { position, size in
