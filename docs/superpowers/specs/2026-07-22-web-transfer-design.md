...
为“简声”增加一个不需要登录注册的网站传书能力。协助者可以在网页输入 App 显示的长期传书码，上传 TXT 或 EPUB 文件；用户随后在 App 内接收并导入到本地书架。
...
- Web 端只通过 App 显示的传书码绑定到 App，不知道用户身份；传书码长期有效，上传会话短期有效。
...
- Supabase Postgres 保存传书身份、传书码和上传记录。
...
```mermaid
flowchart LR
    App["iOS App"] -->|"注册设备/生成传书码"| Functions["Supabase Edge Functions"]
    Web["Web 上传页"] -->|"输入传书码/上传文件"| Functions
...
- 后端只保存传书码 hash，不保存明文。
- 同一传书码最多允许 5 次错误尝试。
- Web 输入传书码后创建短期上传会话；上传文件仍按 72 小时过期。
...
- `expires_at timestamptz`，为空表示长期有效，保留字段便于以后支持手动过期策略
...
- `WebTransferViewModel`：管理传书码、inbox、下载、导入和错误状态。
...
- 传书码使用大字号显示，并支持复制。
- 传书码长期有效，按钮文案为“重新生成传书码”。
- 待接收列表显示文件名、格式、大小和过期时间。
- 下载或导入失败时显示可理解错误，例如“文件已过期”“网络不可用”“格式不支持”“设备空间不足”。
- VoiceOver 至少能读出传书码、长期有效说明、刷新按钮、文件条目、导入按钮和删除按钮。
...
- 传书码错误尝试要增加计数并限制暴力猜测。
- 日志中不能打印传书码明文、deviceSecret 或 signed URL。
